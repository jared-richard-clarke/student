...
#+begin_src
  (*
     ...
     Together these functions perform a depth-first traversal of the DFA's state
     graph while constructing it.
  *)
  fun goto q(c, (Q, δ)) =
      let qₓ = ∂ₓq
      in
  ...
  *)
  ...
  fun mkDFA r =
      let qₒ = ∂εr
      let (Q, δ) = explore ({qₒ}, {}, qₒ)
      let F = { q : q ∈ Q ∧ nullable(q) = ε }
      in (Q, qₒ, F, δ)
#+end_src

*** Implementation Issues
1. The problem of determining when two REs are equivalent, which is used to test if
   *q' ≡ qₓ* is in the ~goto~ function, is computationally expensive.
