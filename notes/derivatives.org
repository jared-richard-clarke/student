...
- C :: A function that approximates derivative classes through structural recursion.
  The algorithm is conservative because it assumes that only those symbols that are related
  in both *C(r)* and *C(s)* are related in *C(r + s)*.

| function    | computation                                    |
|-------------+------------------------------------------------|
| C(ε)        | { Σ }                                          |
| C(S)        | { S, Σ \ S }                                   |
| C(r) ∧ C(s) | { S_{r} ∩ S_{s} : S_{r} ∈ C(r), S_{s} ∈ C(s) } |
...
| step | derivation                          | simplification     |
|------+-------------------------------------+--------------------|
