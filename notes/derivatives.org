...
|------+-------------------------------------|
|    2 | ~match εb* "bb"~                    |
|      | ~(derive ε 'b') + (derive b* 'b')~  |
|      | ~∅ + (derive b 'b')b*~              |
|      | ~∅ + εb*~                           |
|------+-------------------------------------|
|    3 | ~match (∅ + εb*) "b"~               |
|      | ~derive (∅ + εb*) 'b'~              |
|      | ~(derive ∅ 'b') + (derive εb* 'b')~ |
|      | ~∅ + (derive ε 'b')(derive b* 'b')~ |
|      | ~∅ + ∅ + (derive b 'b')b*~          |
|      | ~∅ + ∅ + εb*~                       |
|------+-------------------------------------|
...
#+begin_src haskell
  ...
  derive c f =
      ...
      in case f of
          ...
          Union xs    -> unify (fmap derivation xs)
          Concat x y  -> if nullable x
                         then unify [Concat (derivation x) y, derivation y]
                         else Concat (derivation x) y
          Repeat x    -> Concat (derivation x) f
  ...
  unify :: [Regex] -> Regex
  unify rs =
      let rs         = nub (sort (concatMap flatten rs))
  ...
  type F  = [Regex]
  -- transitions
  type Δ  = [((Regex, Regex), Char)]

  alphabet = ['a'..'z']

  -- Compiles a Regex to a DFA
  compile :: Regex -> (Q, Qo, F, Δ)
  compile regex =
      let (states, edges) = explore ([Regex], []) Regex
          explore graph state = foldl' (goto state) graph alphabet
          goto state (states, edges) character =
              let derived  = derive character state
                  edges    = ((state, derived), character):edges
              in if elem derived states
                 then (states, edges)
                 else explore (derived:states, edges) derived
