  ...
  derive c f =
      ...
      in case f of
          ...
          Concat x y  -> if nullable x
                         then unify [derived (Concat x y), derived y]
                         else derived (Concat x y)
          Union xs    -> unify (fmap derived xs)
          Repeat x    -> derived (Concat x f)
  ...
  compile regex =
      let (states, edges) = explore ([Regex], []) Regex
          ...
          goto state (states, edges) character =
              let derived_state  = derive character state
                  edge = ((state, derived_state), character):edges
              in if elem derived_state states
                 then (states, edge)
                 else explore (derived_state:states, edge) derived_state
