...
- C(r) :: A function that approximates derivative classes. The algorithm
  is conservative because it assumes that only those symbols that are
  related in both *C(r)* and *C(s)* are related in *C(r + s)*.
