...
- Brzozowski derivative :: *u^{-1}S* of a set *S* of strings and a string *u* is the
  the set of all strings obtainable from a string in *S* by cutting off the prefix *u*:
  u^{-1}S = { v ∈ Σ* : uv ∈ S }
