...
The rule *Γ ⊢ e : τ* specifies a procedure that accepts a context and expression
and returns a monotype.
...
Makes the side effects imposed by the procedure *union* explicit by expressing
...
#+begin_example
  ...
  || "mgu" is a side-effect free version of "union", producing a substitution,
...
#+begin_quote
  "The specific algorithm we will be implementing is known as Algorithm W.
   This algorithm differs from Algorithm J by the fact that we build up a
   set of constraints first, then perform unification and substitution.
   In Algorithm J, you would unify and substitute while generating the
   constraints."

  — *Implementing a Hindley-Milner Type System (Part 2)*, 05st
#+end_quote
...
#+begin_src haskell
  ...
  instance Substitutable Scheme where
      tvs (Forall vs t)     = tvs (Set.difference t vs)
      apply s (Forall vs t) = Forall vs $ apply (foldr Map.delete s vs) t

  instance Substitutable Constraint where
      tvs (Constraint t1 t2)     = tvs (Set.union t1 tvs) t2
  ...
  generalize ctx t = Forall (tvs (Set.difference t tvs) (Map.elems ctx)) t
  ...
  solve s ((Constraint t1 t2) : cs) = do
      s1 <- unify t1 t2
      solve (compose s1 s) (apply s1 cs)
