...
- weak head normal form (WHNF) :: An expression that has been evaluated
  to the outermost data constructor or lambda abstraction — the head of
  an expression.
...
In combination with pointer substitution, overwriting the root of a redex
ensures a shared argument need only be computed once.
