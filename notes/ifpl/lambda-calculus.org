...
| *Miranda System Manual*                                  | David Turner                        |
...
#+begin_example
  ...
  where E = expression
        B = filter (boolean expression)
        L = list
        Q = qualifiers (zero or more generators and/or filters)
        v = variable
...
#+begin_quote
  "...[T]he use of let-expressions instead of lambda abstractions has also
   avoided the introduction of *UNPACK-SUM* and *UNPACK-PRODUCT*. If all
   pattern-matching is compiled to case-expressions, then *UNPACK-SUM*
   and *UNPACK-PRODUCT* do not need to be implemented at all!"
...
#+begin_example
  ...
  || -- conformality transformation --
  ||
  || Lazy product-matching ensures the right-hand side of the
  || definition is not evaluated unless at least one of the
  || components of the left-hand tuple is used.
...
- Replaces ~letrec~ expressions with more efficient ~let~ expressions
  wherever possible.

- Sorts mutually recursive definitions into minimal groups.

- Makes type checking possible. A definition in a ~letrec~ expression
  whose right-hand side does not depend on the other names defined in
  the ~letrec~ might confuse the type checker.

- Makes strictness analysis more efficient.
...
#+begin_example
  ...
  node               = variable
  arc                = x depends on y | x -> y
  strongly-connected = x depends on y, y depends on x | x -> y -+
                                                        ^       |
                                                        +- ... -+
  +---------------+
  |               |
...
*Side Note*: The examples below express neither the order nor degree that
a Miranda expression can be transformed into the enriched lambda calculus.
...
#+begin_example
  ...
  || conformality transformation for lazy evaluation
  ...
  head = λxs'.let pair = let v = xs'
                         in CASE-list v (let e = SELECT-SUM-NIL v
                                         in ERROR)
...
- pattern :: Either a variable, a constant, or a constructor of the form
  ~(c p₁ ... pₓ)~ where ~c~ is a constructor of arity ~x~ and ~p₁ ... pₓ~
  are patterns.

- sum pattern :: ~(s p₁ ... pₓ)~. A pattern that matches on a sum type —
  an algebraic data type that can be one of several distinct but related
  constructors, containing zero or more fields.

- product pattern :: ~(t p₁ ... pₓ)~. A pattern that matches on a product type —
  an algebraic data type that has only one constructor and has at least one field.

- simple pattern :: ~(c v₁ ... vₓ)~, where ~v₁ ... vₓ~ are distinct variables.

- nested pattern :: A constructor pattern that contains constructor patterns within itself.

- refutable pattern :: A pattern match that can fail.

- irrefutable pattern :: A pattern composed only of non-repeating identifiers and
  product consttructors of the form *(t p₁ ... pₓ)*, where *p₁ ... pₓ*
  are irrefutable patterns.

- exhaustive pattern :: A pattern that matches all possible cases for its data type.
...
1. function definitions
   ...
   - The main use of pattern matching is in the left-hand side of function
     definitions, providing the right-hand side with names for subcomponents
     of a data structure.

   - More generally, patterns provide case analysis on a function's arguments.

2. conformal definitions

   - *pattern = rhs*

   - If the value of the right-hand side (rhs) matches the structure of the given
     pattern, the variables on the left are bound to the corresponding
     components of the value.

   - If the match fails anywhere, all left-hand variables are undefined.

3. The left of ~<-~ (generators) in list comprehensions.
