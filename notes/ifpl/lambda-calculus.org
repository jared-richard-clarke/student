...
*** Constant, Sum, and Product Expressions
...
#+begin_example
  ...
  ||
  || Variable patterns need not be transformed, because
  || they are already in simple form.
...
#+begin_example
  ...
  || - transforms ->
  addPair = λw.(let x = SELECT-PAIR-1 w
                    y = SELECT-PAIR-2 w
                in (+ x y))
#+end_example
...
- nested pattern :: A constructor pattern that is *not* simple. A pattern
  that contains patterns within itself.
