...
*Side Note*: The examples below follow neither the order nor degree that
a Miranda expression can be transformed into the enriched lambda calculus.
Equivalency denotes transformation.
...
#+begin_example
  ...
  sum = λxs'.CASE-T xs' (let n = SELECT-SUM-NIL xs' in 0)
                        (let x  = SELECT-SUM-CONS-1 xs'
                             xs = SELECT-SUM-CONS-2 xs'
                         in (+ x (sum xs)))
  ...
  reflect = λt.( (UNPACK-SUM-LEAF (λn.LEAF n) t)
               ▯ (UNPACK-SUM-BRANCH (λx.λy.BRANCH (reflect y) (reflect x)) t)
               ▯ ERROR)
  ...
  reflect = λt.case t of
               LEAF n     -> LEAF n
               BRANCH x y -> BRANCH (reflect y) (reflect x)

  || - equivalent ->

  reflect = λt.CASE-T t (let n = SELECT-SUM-LEAF t
                         in LEAF n)
                        (let x = SELECT-SUM-BRANCH-1 t
                             y = SELECT-SUM-BRANCH-2 t
                         in BRANCH (reflect y) (reflect x))
