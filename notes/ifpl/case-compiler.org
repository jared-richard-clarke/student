...
do not backtrack on multiple, nested, and non-uniform patterns.

*Side Note*: To keep the ~match~ compiler simple, the author, Philip Wadler,
assumes constant patterns have been replaced by conditional equations.
...
#+begin_example
  ...
  ||
  || Side Note: The list of variables is the same length
  ||            as each list of patterns, 
...
#+begin_example
  ...
  || Variable patterns need not be the same identifier.
 ...
 || === Mixture Rule ===
 ||
 || The equation list is partitioned into "x" lists so that
 || every equation begins with a matching rule.
 ||
 || "match" recursively builds out case clauses for "qs₁" through "qsₓ"
 || bottoming out for either successful matches or default value "E".
...
A set of equations is *uniform* if one of the following three conditions hold:

1. All equations begin with a variable pattern. Applying the variable rule
   yields a new, uniform set of equations.

2. All equations begin with a constructor pattern. Applying the constructor
   rule yields a new, uniform set of equations.

3. All equations have an empty list of patterns and there is zero or one
   equations in the set.

Changing the order of uniform equations does not change the meaning
of their definition. Compilation of such definitions is simpler and
more efficient.
...
#+begin_src
  ...
  match number (v:vs) equations default
      = foldr (matchMix number (v:vs)) default (partition isVariable equations)

  matchMix number variables equations default
