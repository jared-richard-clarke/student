...
on nested and multiple patterns.
...
#+begin_example
  || === The Variable Rule ===
  ||
  || If every equation begins with a variable pattern, then
  || the call of match will have this form:
  ...
  ||
  || A list of equations that begin with a constructor
  || is equivalent to a case expression derived by grouping
  || together all equations that begin with the same
  || constructor. Within each group, new variables are
  || introduced corresponding to each field of the
  || constructor.
...
** Miranda: Pattern Matching Compiler
