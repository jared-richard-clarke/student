...
3. For sets *R* in *P(Q)* and input characters *c ∈ Σ*, the transition function *δ_{n}* of the *DFA*
   is defined in terms of the transition function *δ* of the *NFA* by *δ_{n}(R,c) = U_{r ∈ R} E(δ(r,c))*.
...
*Capture* into the function *addthread(threadlist, thread, sp)* and calling said function recursively
to add targets of said instructions to worklists *clist* and *nlist* would ensure highest to lowest
thread priority. Also:

- If a match is found, lower-priority threads in *clist* should be ignored.
  That is, *clist* is cleared of states.

- Higher priority threads should be able to match possibly-longer sections of input.
  That is, higher-priority threads might continue to build up states in *nlist*
  via recursive calls to *addthread* after *clist* has been cleared.

#+begin_quote
  "To implement unanchored matches, many implementations try for a match at position 0,
   then try for a match at position 1, and so on. If the regexp engine scans to the end
   of the string before failing to find a match, wrapping it in this loop makes the
   unanchored search quadratic in the length of the text. In VM-based implementations,
   a more efficient way to do an unanchored search is to put the compiled code for ~.*?~
   at the beginning of the program. This lets the VM itself implement the unanchored
   search in a single linear-time pass."
...
#+begin_src c
  ...
  // Thread thread(Instruction *pc, char **captured) {
  //     Thread t = { pc, captured };
  //     return t;
  // }
  ...
  // int pike_vm(Instruction *program, char *input, char **captured)
      // Program counter
      Instruction *pc;
      // String pointer
      char *sp;
      ...
      // addthread(clist, thread(program, captured));
      ...
      for (sp = input; *sp; sp += 1) {
          for (int i = 0; i < clist.n; i += 1) {
              ...
              case Char:
                  ...
                  // addthread(nlist, thread(pc + 1, t.captured));
                  break;
              case Match:
                  // memmove(captured, t.captured, sizeof t.captured);
                  return 1;
              case Jump:
                  addthread(clist, thread(pc->x));
                  // addthread(clist, thread(pc->x, t.captured));
                  break;
              case Split:
                  addthread(clist, thread(pc->x));
                  // addthread(clist, thread(pc->x, t.captured));
                  addthread(clist, thread(pc->y));
                  // addthread(clist, thread(pc->y, t.captured));
                  break;
           // case Capture:
           //     t.captured[t->pc.i] = sp;
           //     addthread(clist, thread(pc->x, t.captured));
           //     break;
              }
          }
