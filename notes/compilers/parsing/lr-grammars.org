...
Each parser state has its own row. Each possible next symbol has its own column.
Some combinations of state and next symbol are impossible for valid input streams.
These blank cells trigger syntax errors.
