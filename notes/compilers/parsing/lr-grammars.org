...
Each parser state has its own row. Each possible symbol has its own column. Some combinations
of state and symbol are impossible for valid input streams. These blank cells trigger syntax errors.
