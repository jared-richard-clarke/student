...
- B. Ford, "Parsing Expression Grammars: A Recognition-Based Syntactic Foundation." Available: https://bford.info/pub/lang/peg.pdf

- R. Ierusalimschy, "A text pattern-matching tool based on Parsing Expression Grammars,"
  Software: Practice and Experience, vol. 39, no. 3, pp. 221–258, Mar. 2009, doi: https://doi.org/10.1002/spe.892.
...
#+begin_example
  ...
  grammar      <- (non-terminal '<-' space pattern)+
  pattern      <- alternative ('/' space alternative)*
  alternative  <- ([!&]? space suffix)+
  suffix       <- primary ([*+?] space)*
  primary      <- '(' space pattern ')' space / '.' space / literal /
                char-class / non-terminal !'<-'
  literal      <- ['] (!['] .)* ['] space
  char-class   <- '[' (!']' (. '-' . / .))* ']' space
  non-terminal <- [a-zA-Z]+ space
  space        <- [ \t\n]*
#+end_example
