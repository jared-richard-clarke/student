...
#+begin_quote
  "It may seem that we could put the *End* instruction after the initial Call, without the jump.
   However, an important characteristic of LPEG is that grammars result in regular patterns.
   We can get the result of a grammar and use it like any other pattern, even inside other
   grammars. To allow this composability, grammars follow the rule that a pattern ends with
   its only *End* instruction."
