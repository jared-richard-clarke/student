...
#+begin_src c
  ...
  static int pushcapture (CapState *cs) {
    lua_State *L = cs->L;
    // Records number of values left stack by capture.
    int res;
    ...
    switch (captype(cs->cap)) {
      // Match epsilon and capture index position in string.
      case Cposition: {
      ...
      // Match epsilon and return all values provided outside string as captures.
      case Cconst: {
      ...
      // Match epsilon and return the nth extra argument in "lpeg.match" as capture.
      case Carg: {
        int arg = (cs->cap++)->idx;
      ...
      // Capture subject's substring that matches pattern. Substring can be followed
      // by nested captures.
      case Csimple: {
        // Push all values generated by the captures nested within
        // the current capture followed by the primary substring.
        int k = pushnestedvalues(cs, 1);
        // Rotate primary substring match to the first result.
        lua_insert(L, -k);
        res = k;
        break;
      }
      ...
      // For string captures, place intermediate results in buffer before pushing
