...
- ~lpeg.match(pattern, subject, index?)~ :: Attempts to match the given pattern against
  the subject string. If successful, returns either the index of the first character after
  the match or the values captured from the subject string. An optional numeric argument
