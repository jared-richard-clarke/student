...
#+begin_src c
  ...
  // TestChar x L, S[i] = x [ (p, i, e, c) -> (p+1, i+1, e, c) ]
  ...
  // Choice L d [ (p, i, e, c) -> (p+1, i, (p+L, i-d, c):e, c) ]
  case IChoice: {
    if (stack == stacklimit) {
...
#+begin_src c
  ...
  // TestCharset X L, S[i] ∈ X [ (p, i, e, c) -> (p+1, i+1, e, c) ]
  ...
  // TestAny n L, i+n ≤ |S| [ (p, i, e, c) -> (p+1, i+1, e, c) ]
...
#+begin_src lua
  ...
  Grammar = Pattern{
      "A";
       A = Pattern("abc") + Any * Variable("A")
  }

  -- === Virtual Machine ===
  --
  -- L1: Choice L2 <- Expensive storing and restoring of state
