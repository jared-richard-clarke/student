...
#+begin_example
  ...
  o: Grammar × V → N = Σ (|Π(g, x, Aj)| + 1)
                      j=1       ^         ^ The size of the "Return" instruction.
                                |
                                  Position relative to the beginning of the subprogram.
#+end_example
...
#+begin_src c
  ...
  // An unsigned integer large enough to index any subject.
  // "size_t" would double the size of the captures array on a 64-bit machine.
  // "uint" = "unsigned int"
