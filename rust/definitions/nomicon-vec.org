...
#+begin_src rust
  ...
  impl<T> IntoIterator for Vec<T> {
      ...
      fn into_iter(self) -> IntoIter<T> {
          unsafe {
              ...
              // Prevents "Vec" from freeing its own memory at end of scope.
              mem::forget(self);
  ...
  impl<T> RawValIter<T> {
      unsafe fn new(slice: &[T]) -> Self {
          RawValIter {
              start: slice.as_ptr(),
              // For zero-sized types, ".add()" does not advance the pointer address.
              // Instead the pointer must be cast to usize so that the length of the
              // slice can be added directly.
              end: if mem::size_of::<T>() == 0 {
                  ((slice.as_ptr() as usize) + slice.len()) as *const _
              // If the slice has no elements, the "end" pointer is set equal to "start".
              } else if slice.len() == 0 {
                  slice.as_ptr()
              } else {
  ...
  impl<T> Iterator for RawValIter<T> {
      ...
      fn next(&mut self) -> Option<T> {
          if self.start == self.end {
              ...
              unsafe {
                  // ".offset(1)" is a no-op for zero-sized types. Convert pointer to usize
                  // and increment byte by byte.
                  if mem::size_of::<T>() == 0 {
                      self.start = (self.start as usize + 1) as *const _;
                      // Zero-sized types have no data and occupy no memory. Return a dangling
                      // pointer instead.
      ...
      // .size_hint() -> (lower bound, upper bound)
