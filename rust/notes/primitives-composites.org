...
- trait object :: Referred to as ~dyn Trait~, the original type is erased and replaced by a
  pointer to a *virtual table (vtable)*, which allows runtime reflection.

- slice :: A view into some contigous storage — usually an array or ~Vec~. A slice is a pointer
  paired with the number of elements in the underlying data.
