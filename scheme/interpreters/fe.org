...
#+begin_src c
  ...
  // === Error Handling ===
  // By default, an error and stack trace are printed to standard error,
  // and the program exits with an EXIT_FAILURE.
  //
  // Can provide custom error handlers. "longjmp" can be used to exit said handlers
  // and re-enter the context in a safe state.
