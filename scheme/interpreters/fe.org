...
a pre-allocated block of memory and its size. Call ~fe_Close~ at the end of a Fe program to
ensure all ~ptr~ objects are properly garbage collected.

#+begin_src c
  int size   = 1024 * 1024;
  void* data = malloc(size);

  feContext* ctx = fe_open(data, size);
...
#+begin_src c
  ...
  */
  for (;;) {
    ...
    /*
       Restore GC stack. It now contains both
       a read object and an evaluated expression.
    */
...
#+begin_src c
  ...
  /* Discard all objects pushed to GC stack. */
  fe_restoregc(ctx, gc);
#+end_src
