...
| notation     | definition                                                                |
|--------------+---------------------------------------------------------------------------|
| ~symbol~     | A Fe identifier.                                                          |
| ~value~      | A Fe value, often the output of a Fe expression.                          |
| ~condition~  | Predicate expression.                                                     |
| ~then~       | Consequent expression. Evaluated if ~condition~ evaluates to truth value. |
| ~else~       | Alternative expression. Evaluated if ~condition~ evaluates ~nil~.         |
| ~expression~ | An unevaluated Fe expression.                                             |
| ~car~        | The head of a cons cell. A Fe value or expression.                        |
| ~cdr~        | The tail of a cons cell. A Fe value or expression.                        |
...
- ~(fn symbol* expression+)~ :: Defines a function procedure.

- ~(mac symbol* expression+)~ :: Defines a macro expander.
...
- ~(cons car cdr)~ -> ~(car cdr)~ :: Creates a cons cell, otherwise known as a pair.

- ~(car (car cdr))~ -> ~car~ or ~nil~ :: Returns the ~car~ of a cons cell, or ~nil~ if the cons cell is ~nil~.

- ~(cdr (car cdr))~ -> ~cdr~ or ~nil~ :: Returns the ~cdr~ of a cons cell, or ~nil~ if the cons cell is ~nil~.

- ~(setcar (car cdr) value)~ :: Sets the ~car~ of a cons cell to ~value~.

- ~(setcdr (car cdr) value)~ :: Sets the ~cdr~ of a cons cell to ~value~.

- ~(list value*)~ -> ~(value*)~ :: Creates a cons list of zero or more values. The bottom
  value of a proper list is ~nil~.
...
- ~(atom value)~ -> ~t~ or ~nil~ :: Returns true if value is not cons cell. Otherwise returns ~nil~.
