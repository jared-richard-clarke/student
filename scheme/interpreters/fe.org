...
#+begin_src c
  ...
  */
  fe_Object* fe_symbol(fe_Context *ctx, const char *name) {
    ...
    /*
      Creates new object. Pushes object pointer to "symlist".
    ...
    switch (type(obj)) {
      ...
      */
      case FE_TNUMBER:
        ...
        break;
      /*
        Writes a Fe list. Start at an open parenthesis and both loops and recurses
        through its contents. Terminates with a closing parenthesis.
      */
      case FE_TPAIR:
        fn(ctx, udata, '(');
        for (;;) {
      ...
      case FE_TSYMBOL:
        ...
        break;
      /* Writes a Fe string. If "qt" is set to true, write a string wrapped in quotes. */
      case FE_TSTRING:
        if (qt) { fn(ctx, udata, '"'); }
        while (!isnil(obj)) {
          int i;
  ...
  */
  ...
  /* Extract a number from a Fe number object. */
