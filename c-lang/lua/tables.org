...
Lua encloses a chunk within the body of an anonymous function. A module is an anonymous
function that provides side effects and/or returns a value when called. Lexical scoping
ensures only returned values can access data within the lexical environment of their
parent function.
