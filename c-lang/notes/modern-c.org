...
- Don't cast the return type of ~malloc~ and related allocation functions. Assignment provides
  the effective type and value. Not only is the cast superfluous, but doing an explicit conversion
  can even be harmful when ~stdlib.h~ is not included. Older C compilers might assume a return
  of ~int~ and trigger an erroneous conversion from ~int~ to a pointer type.
