...
*** Numbers Loop: Assembly Output ~x86-64 gcc 14.2~
...
#+begin_src asm
  add:
          ...
          pop     rbp                           ; <-- Return from "add" function.
          ret                                   ; <+
  main:
          push    rbp                           ; <+
          mov     rbp, rsp                      ;  |
...
*** Copy String: Assembly Output ~x86-64 gcc 14.2~
...
#+begin_src asm
  copy_string:
          ...
          mov     rax, QWORD PTR [rbp-24]  ; <-- Move "destination" pointer into "saved" pointer.
          mov     QWORD PTR [rbp-8], rax   ; <+
          jmp     .L2                      ; <-- Jump to "while" loop.
  .L3:
          mov     rax, QWORD PTR [rbp-32]  ; <+
          movzx   edx, BYTE PTR [rax]      ;  |- Copy value from "source" to "saved".
          mov     rax, QWORD PTR [rbp-8]   ;  |
          mov     BYTE PTR [rax], dl       ; <+
          add     QWORD PTR [rbp-32], 1    ; <-- Increment pointer to "source".
          add     QWORD PTR [rbp-8], 1     ; <-- Increment pointer to "saved".
  .L2:
          mov     rax, QWORD PTR [rbp-32]  ; <+
          movzx   eax, BYTE PTR [rax]      ;  |- If predicate is satisfied, jump to "while" block.
          test    al, al                   ;  |
          jne     .L3                      ; <+
          mov     rax, QWORD PTR [rbp-8]   ; <+- Append '\0' to end of "saved" array.
          mov     BYTE PTR [rax], 0        ; <+
          mov     rax, QWORD PTR [rbp-8]   ; <-- Return "saved" pointer from "copy_string".
          pop     rbp                      ; <+- Return to caller from "copy_string".
          ret                              ; <+
  .LC0:
