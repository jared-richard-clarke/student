...
- The conversion from ~void*~, the return type of ~malloc~ and related functions, to the target type
  is automatic. Assignment provides the effective type and value. Not only is the cast superfluous,
  but doing an explicit conversion can even be harmful when ~stdlib.h~ is not included.
  Older C compilers then assume a return of ~int~ and trigger an erroneous conversion from ~int~
  to a pointer type.
