...
*Side Note*: Setting the storage class of ~numbers~ to ~static~ pushes ~numbers~ out of the
stack and into its own dedicated region of memory. Declaring and initializing ~numbers~ outside
of ~main~ accomplishes the same.
...
#+begin_src asm
  ...
  .L4:
          ...
          jle     .L5                           ; <-- Jumps to body of "for" loop if condition satisfied.
